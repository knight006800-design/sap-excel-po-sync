# -*- coding: utf-8 -*-
"""SAP 화면 마우스 보정 마법사 (단계별 안내 그림 표시)."""
...
from paths_util import guide_images_dir, load_config, save_config
...
STEPS = [
    (
        u"material_tl",
        u"1/5  자재코드 열의 '왼쪽 위'(첫 데이터 셀 왼쪽 상단)를 클릭하세요",
        u"guide_material_tl.png",
    ),
    (
        u"material_br",
        u"2/5  자재코드 열의 '오른쪽 아래'(마지막 보이는 행 오른쪽 하단)를 클릭하세요",
        u"guide_material_br.png",
    ),
    (
        u"qty_x",
        u"3/5  오더수량 열의 아무 셀 '중앙'을 클릭하세요 (X좌표 사용)",
        u"guide_qty_x.png",
    ),
    (
        u"row1_y",
        u"4/5  첫 번째 데이터 행의 중앙을 클릭하세요",
        u"guide_row1_y.png",
    ),
    (
        u"row2_y",
        u"5/5  두 번째 데이터 행의 중앙을 클릭하세요 (행 높이 계산)",
        u"guide_row2_y.png",
    ),
]
...
class CalibrateWizard(object):
    ...
    def start(self):
        messagebox.showinfo(
            ...
            u"안내 창의 그림과 설명을 보고 진행하세요.\n"
            u"취소: ESC",
            parent=self.parent,
        )
        self.win = tk.Toplevel(self.parent)
        self.win.title(u"보정 진행 — 클릭 위치 안내")
        self.win.attributes("-topmost", True)
        self.win.geometry("540x420+120+40")
        self.label = tk.Label(
            self.win,
            text=STEPS[0][1],
        ...
        self.img_label = tk.Label(self.win, bg="#eeeeee")
        self.img_label.pack(padx=10, pady=4)
        self.status = tk.Label(self.win, text=u"클릭 대기 중...", fg="blue")
        self.status.pack(pady=6)
        self._show_guide(STEPS[0][2])
        threading.Thread(target=self._run, daemon=True).start()

    def _show_guide(self, filename):
        path = os.path.join(guide_images_dir(), filename)
        if not os.path.isfile(path):
            self.img_label.config(image="", text=u"(안내 그림 없음: {0})".format(filename))
            self._photo = None
            return
        try:
            img = Image.open(path)
            ...
            self.img_label.config(image="", text=u"그림 로드 실패: {0}".format(e))
            self._photo = None
    ...
    def _run(self):
        try:
            for key, text, guide in STEPS:
                if self._stop:
                    break

                def _update(t=text, g=guide):
                    self.label.config(text=t)
                    self.status.config(text=u"클릭 대기 중...", fg="blue")
                    self._show_guide(g)
            ...
            self.win.after(
                0, lambda: messagebox.showerror(u"보정 오류", str(e), parent=self.parent)
            )
        finally:
            self.win.after(0, self._close)
