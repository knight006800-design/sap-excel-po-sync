# -*- coding: utf-8 -*-
"""SAP 화면 마우스 보정 — 주모니터에 빨간점 안내 그림 (＋마커 없음)."""
...
STEPS = [
    (
        u"material_tl",
        u"1/5  자재코드 · 왼쪽 위",
        u"그림 빨간 점 = 첫 행 자재코드 칸의 왼쪽 위 모서리",
        u"guide_material_tl.png",
    ),
    (
        u"material_br",
        u"2/5  자재코드 · 오른쪽 아래",
        u"그림 빨간 점 = 마지막 행 자재코드 칸의 오른쪽 아래 모서리",
        u"guide_material_br.png",
    ),
    (
        u"qty_x",
        u"3/5  오더수량 · 정중앙",
        u"그림 빨간 점 = 첫 행 오더수량 숫자 정중앙",
        u"guide_qty_x.png",
    ),
    (
        u"row1_y",
        u"4/5  첫 행 · 세로 중앙",
        u"그림 빨간 점 = 첫 데이터 행의 세로 한가운데",
        u"guide_row1_y.png",
    ),
    (
        u"row2_y",
        u"5/5  둘째 행 · 세로 중앙",
        u"그림 빨간 점 = 둘째 데이터 행의 세로 한가운데",
        u"guide_row2_y.png",
    ),
]
...
class CalibrateWizard(object):
    ...
    def _show_guide(self, filename):
        ...
        if not os.path.isfile(path):
            self.img_label.config(image="", text=u"그림 없음: {0}".format(filename))
            self._photo = None
            return
        try:
            img = Image.open(path)
    ...
    def _tick_coord(self):
        ...
        try:
            ...
            pass
        try:
            self.win.after(30, self._tick_coord)
        except Exception:
            pass
