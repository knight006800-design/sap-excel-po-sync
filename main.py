...
from paths_util import APP_NAME, app_dir, load_config, save_config
...
class App(object):
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(u"구매오더 수량 동기화 (SAP ↔ Excel)")
        ...
        tk.Label(frm, text=u"엑셀 파일 연결", font=("Malgun Gothic", 11, "bold")).pack(anchor="w")
        row = tk.Frame(frm)
        row.pack(fill="x", pady=4)
        self.excel_var = tk.StringVar(value=self.cfg.get("excel_path") or "")
        tk.Entry(row, textvariable=self.excel_var).pack(side="left", fill="x", expand=True)
        tk.Button(row, text=u"찾아보기", command=self.browse_excel, width=10).pack(side="left", padx=4)

        btn_row = tk.Frame(frm)
        btn_row.pack(fill="x", pady=8)
        tk.Button(btn_row, text=u"1. SAP 화면 보정", command=self.do_calibrate, width=18).pack(
            side="left", padx=2
        )
        tk.Button(btn_row, text=u"2. 동기화 실행", command=self.do_run, width=18, bg="#dcebff").pack(
            side="left", padx=2
        )
        tk.Button(btn_row, text=u"중지 (ESC)", command=self.do_stop, width=12).pack(side="left", padx=2)
        tk.Button(btn_row, text=u"구동점검 폴더", command=self.open_log_folder, width=14).pack(
            side="left", padx=2
        )
        ...
        tk.Label(
            ...
            u"실행 중 마우스/키보드를 만지지 마세요. 긴급중지=ESC. 로그=구동점검.txt",
            justify="left",
            fg="#333",
        ).pack(anchor="w", pady=4)

        tk.Label(frm, text=u"실행 로그", font=("Malgun Gothic", 10, "bold")).pack(anchor="w")
    ...
    def do_calibrate(self):
        if self._running:
            messagebox.showwarning(u"실행 중", u"동기화 실행 중에는 보정할 수 없습니다.")
            return
    ...
    def do_run(self):
        ...
        if not os.path.isfile(path):
            messagebox.showerror(u"파일 없음", u"엑셀 파일을 찾을 수 없습니다:\n{0}".format(path))
            return
        ...
        try:
            ...
            if stats:
                self.root.after(
                    ...
                    lambda: messagebox.showinfo(
                        ...
                        u"상세는 구동점검.txt 를 확인하세요.".format(**stats),
                    ),
                )
            ...
            self.root.after(
                ...
                lambda: messagebox.showerror(
                    u"오류",
                    u"{0}\n\n구동점검.txt 와 캡처 PNG를 다른 PC에서 가져와 전달해 주세요.".format(e),
                ),
            )
        finally:
            self._running = False
